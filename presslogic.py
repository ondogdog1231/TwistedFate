...
class presslogic():
    def __init__(self, url):
        ...
        self.headers = {
            ...
            'referer': 'http://businessfocus.presslogic.com'}
    ...
    def detail(self,  detailUrl):
        ...
        rawContent = soup.find("div", {"class", "entry-content"}).findAll("p")
        content = ""
        ...
        try:
            with closing(self.cnx.cursor()) as cursor:
                cursor.execute(
                    "INSERT INTO finance(`platform_id`,`title`,`content`,`href`,`created_time`,`updated_time`) VALUES (%s,%s,%s,%s,%s,%s)",
                    (7, title, content, detailUrl, int(time.time()), int(time.time()),))
            self.cnx.commit()
        ...
        time.sleep(3)
for i in range(1,8):
    t = presslogic("http://businessfocus.presslogic.com/page/" +str(i))
    t.run()
